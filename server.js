...
async function blockViaBrowser(booking) {
  ...
  // Capture auth token from any authenticated API request after login
  page.on('request', req => {
    const url = req.url(), method = req.method();
    if (url.includes('playtomic.io/api') && ['GET','POST','PATCH','PUT'].includes(method)) {
  ...
  try {
    ...
    // Wait for token to be captured from page load requests
    const start = Date.now();
    while (!cachedAuthToken && Date.now() - start < 8000) {
      await page.waitForTimeout(200);
    }

    if (!cachedAuthToken) {
      throw new Error('Logged in but no Bearer token captured within 8s');
    }

    // Now use API directly with the captured token
    await blockViaAPI(booking);
    console.log('✅ Block created via API after browser login.');
