 ...
 * Token lives in memory for ~55 min. On expiry → re-login via browser.
 * Browser is closed BEFORE API call to free memory.
 *
...
// ── BROWSER LOGIN ─────────────────────────────────────────────────────────────
// Browser is used ONLY to capture the token.
// It is fully closed before any API call is made.
...
async function _doBrowserLogin() {
  ...
  // Capture token from any authenticated API request
  page.on('request', req => {
  ...
  try {
    ...
    // Always close browser fully before returning — frees all memory for API call
    await browser.close();
